...
if user_input:
    ...
    if combined_embeddings.shape[0] > 2:
        st.write("**Visualizing Combined Embeddings using PCA:**")
        st.write("**2D:**")

        pca = PCA(n_components=2)
        embeddings_2d = pca.fit_transform(combined_embeddings.numpy())
        fig, ax = plt.subplots()
    ...
    3. **Residual Connection**: Adds the attention output back to the input state.
    ...
    st.latex(r''' \text{Layer-Normalized Output} (H_{\text{norm}}) = \text{Input} \times W_{\text{L1}} + b_{\text{L1}} ''')
    ...
    st.write(f"**Layer-Normalized Hidden States Before MLP:** Shape {hidden_states_norm_2.shape}")
    st.latex(r''' \text{Layer-Normalized Output} (H_{\text{norm}}) = \text{Residual\_1} \times W_{\text{L2}} + b_{\text{L2}} ''')
    ...
    st.write("## **Step 9: Second Residual Connection**")
    ...
    st.latex(r''' \text{Layer\_Output} = \text{Residual\_1} + \text{MLP\_Output} ''')
    ...
    pipeline_data.append({
        "Step": "Embeddings",
        "Value": f"Generated by adding token and position embeddings (first {N} dimensions displayed)"
    })
    ...
    pipeline_data.append({
        ...
        "Value": f"Visualized Layer {layer}, Head {head}"
    })
